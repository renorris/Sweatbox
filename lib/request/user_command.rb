...
class RequestUserCommand
  ...
  def process
    ...
    if command.match /^broadcast/
      ...
      if @admin
        ...
        @response.push("#TMSERVER:#{@session_cache['callsign']}:You are not permitted to use instructor commands!")
      end
      return @response
    elsif command.match /^interval/
      interval = command.split(" ")[1]
      if !interval.nil? && is_i?(interval) && interval.to_i <= 5 && interval.to_i >= 1
        @session_cache['position_update_interval'] = interval
        @response.push("#TMSERVER:#{@session_cache['callsign']}:Position update interval set to #{interval} seconds.")
      else
        @response.push("#TMSERVER:#{@session_cache['callsign']}:Interval must be 1 to 5 seconds, no decimals.")
      end
      return @response
  ...
  def remove_aircraft(aircraft)
    ...
    @response.push_broadcast_line(BroadcastLine.new("#DP#{aircraft}:goodbai world", "ranged"))
  ...
  def broadcast(command)
    ...
    @response.push_broadcast_line(BroadcastLine.new("#TM#{@session_cache['callsign']}:*:#{message}", "global"))
  ...
  def add_aircraft(command)
    ...
    if !callsign_already_exists
      ...
      if callsign == "VMC"
        @response.push("#TMSERVER:#{@session_cache['callsign']}:Veektar Miek Charlay, hOld sHorT oF thE rUnwaY anD stAnD bAi for TaXi clEaRance")
      end
    else
